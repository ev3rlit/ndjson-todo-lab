...
기본 `docker-compose.yml`은 로컬에서 바로 실행할 수 있도록 Docker named volume을 사용합니다. 실제 VM1에서는 `scripts/vm1/setup.sh`가 `docker-compose.vm1.yml` override를 함께 적용해서 `/srv/ndjson-todo/...` 호스트 경로를 bind mount하고, 컨테이너 엔진은 `podman compose`를 사용합니다.
