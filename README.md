...
이제 레포는 모노레포로 정리되어 있습니다. Todo 서비스는 `apps/todo-service`, 발표 자료는 `apps/slides`에서 함께 관리합니다.
...
애플리케이션과 발표 자료를 각각 `apps` 아래에서 관리합니다.
...
기본 `docker-compose.yml`은 로컬에서 바로 실행할 수 있도록 Docker named volume을 사용합니다. 실제 VM1에서는 `scripts/vm1/setup.sh`가 `docker-compose.vm1.yml` override를 함께 적용해서 `/srv/ndjson-todo/...` 호스트 경로를 bind mount하고, 컨테이너 엔진은 `podman-compose`로 고정합니다.
...
처음 실행은 위 두 개만 하면 됩니다. `setup.sh`가 내부에서 필요한 하위 스크립트를 모두 호출하므로, VM2에서 `up.sh` 같은 추가 명령을 따로 실행할 필요는 없습니다.

`setup.sh`는 실행할 때마다 `vm1.env.example -> vm1.env`, `vm2.env.example -> vm2.env`를 강제로 덮어쓴 뒤 진행합니다.

`NFSv4` 기준으로는 VM2가 `/srv/nfs/ndjson-todo`를 `fsid=0`으로 export하고, VM1은 `con2:/`를 `/mnt/ndjson-todo-nfs`에 mount하는 구성을 사용합니다.

### 확인용 명령

- VM2 상태 확인: `./scripts/vm2/status.sh`
- VM1 상태 확인: `./scripts/vm1/status.sh`
- VM1 수동 백업 테스트: `./scripts/vm1/backup-now.sh`
...
`setup.sh`는 NFS 클라이언트 설치, NFS 마운트, 서비스 디렉터리 준비, backup cron 등록, `podman-compose` 기동까지 한 번에 처리합니다. 수동 백업 테스트는 `scripts/vm1/backup-now.sh`로 할 수 있습니다.
...
- 이번 실습의 앱 컨테이너와 로드밸런서 컨테이너는 모두 non-root 사용자로 실행되도록 구성합니다.
- 이 구성은 low port bind 문제를 피하기 위해 컨테이너 내부 포트를 `8080`으로 통일합니다.
- bind mount를 쓰는 실제 VM1 환경에서는 호스트 디렉터리 권한이 맞지 않으면 `permission denied`가 바로 발생합니다.
- `NFS`를 붙이면 rootless 컨테이너의 UID/GID와 export 권한이 어긋나서 쓰기 실패가 날 수 있습니다.
- 로그 디렉터리와 데이터 디렉터리는 rootless 컨테이너가 직접 써야 하므로, 디렉터리 소유권이나 mode 설정이 실습 포인트가 됩니다.
- `podman` rootless 환경에서는 SELinux label 때문에 bind mount에 `:Z` 같은 옵션이 빠지면 곧바로 접근 실패가 발생할 수 있습니다.
- 이번 실습에서는 이런 문제를 직접 관찰하기 위해 rootless 컨테이너 구성을 유지하되, 더 고급한 운영 자동화는 범위에서 제외합니다.
